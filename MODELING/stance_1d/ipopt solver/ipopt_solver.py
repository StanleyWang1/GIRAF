"""Solver for IPOPT/IPYOPT programs

Originally from Thomas Lew (thomas.lew@stanford.edu)

Usage: Construct a inherited Program class instance and pass it into the solver
"""
...
from reachbot_manipulation.optimization.ipopt_problem import Program
...
class Solver:
    def __init__(
        ...
        program: Program,
        name: str = "ipopt",
        delta_equality_relaxation: int = 0,
        verbose: bool = False,
    ):
        ipopt_print_level = 0
        if verbose:
        ...
        self._x_bounds_low = -np.ones(nvar, dtype=np.float_) * 1e3
        self._x_bounds_up = np.ones(nvar, dtype=np.float_) * 1e3
    ...
    def gL_gU(self, x: jnp.array) -> np.array:
        ...
        # constraints_l[num_equality_constraints:] = -1e1
        # constraints_u[num_equality_constraints:] = 1e1
    ...
    @partial(jit, static_argnums=(0,))
    def lagrange_dot_g(self, x: jnp.array, lagrange: jnp.array) -> float:
        return jnp.dot(lagrange, self.g(x))

    @partial(jit, static_argnums=(0,))
    def hessian_lagrange_dot_g(self, x: jnp.array, lagrange: jnp.array) -> jnp.array:
        nvar = self._program.num_variables
        hess = jacfwd(jacfwd(self.lagrange_dot_g))(x, lagrange)[:nvar, :nvar]
        return hess

    def eval_f(self, x: np.array) -> float:
        return self.f(x)

    def eval_grad_f(self, x: np.array, out: np.array) -> np.array:
        out[:] = self.f_dx(x)
        return out

    def eval_g(self, x: np.array, out: np.array) -> np.array:
        out[:] = self.g(x)
        return out

    def eval_jac_g(self, x: np.array, out: np.array) -> np.array:
        out[:] = self.g_dx(x).flatten()
        return out

    def eval_h(
        self, x: np.array, lagrange: np.array, obj_factor: np.array, out: np.array
    ) -> np.array:
        nvar = self._program.num_variables
        hess_cost = self.f_ddx_hessian(x)
        hess_lagrange_prod = self.hessian_lagrange_dot_g(x, lagrange)
    ...
    def get_sparsity_indices(self):
        ...
        jac_g_sparsity_indices = (indices_1.flatten(), indices_2.flatten())
        indices_1, indices_2 = np.tril_indices(nvar)
        h_sparsity_indices = (indices_1.flatten(), indices_2.flatten())
    ...
    def define_optimization_problem(self):
        ...
        (eval_jac_g_sparsity_indices, eval_h_sparsity_indices) = sparsity_indices
        # Precompile
    ...
    def solve(self):
        ...
        x, obj, status = self.nlp.solve(
            x0, mult_g=np.zeros(ncon), mult_x_L=np.zeros(nvar), mult_x_U=np.zeros(nvar)
        )
